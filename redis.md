...
**list相当于列表，可以有重复值，底层数据结构实现是双向链表quicklist**
```html
> rpush list-key item
(integer) 1
...
**hash表，底层数据结构实现的是Dict+skiplist，解决冲突的方法是链地址法，数组加链表的实现**
```html
...
**有序集合，底层数据结构实现的是skiplist,redis最新版本实现的是listpacks**
```html
```html
...
可以使用 Redis 自带的 SETNX(set if not exit) 命令实现分布式锁，并设置锁失效时间，防止加锁客户端奔溃，未释放锁。除此之外，还可以使用官方提供的 RedLock 分布式锁实现。
